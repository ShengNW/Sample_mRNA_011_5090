...
def train_one_epoch(model, loader, optimizer, device, criterion, num_classes,
...
def run_training(cfg: Dict[str, Any]):
    ...
    best_metric = -1.0; best_path = os.path.join(ckpt_dir, 'best.pt')
    num_classes = int(cfg['data']['num_classes'])
    ...
    for ep in range(1, warmup_epochs+1):
        ...
        tr = train_one_epoch(model, train_loader, optimizer, device, criterion,
                             num_classes, scaler, amp_enabled, head_only=False)
        va = evaluate(model, val_loader, device, criterion)
        metric = va['acc']
        print(f"[Epoch {ep:03d}] train_loss={tr['loss']:.4f} train_acc={tr['acc']:.4f} "
    ...
    if probe_head_epochs > 0:
        ...
        for ep in range(1, probe_head_epochs+1):
            ...
            tr = train_one_epoch(model, train_loader, optimizer, device, criterion,
                                 num_classes, scaler, amp_enabled, head_only=True)
            va = evaluate(model, val_loader, device, criterion)
